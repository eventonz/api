...
**Worker**: same PG/REDIS/JWT/RR/DO_SPACES + `PUSH_DRY_RUN=0, PUSH_LANG_TOPICS (unset=per-language), PG_POOL_MAX=5, WEB_PORT=3000, WEB_PASSWORD, EVENTOAPI_URL, EVENTOAPI_PUSH_KEY (server key), ADMIN_ALERT_EMAILS, RESEND_API_KEY, MAIL_FROM, CMS_URL, MAX_LIVE_H=20, AUTO_STOP_H=36, ALERT_REPEAT_H=24, PULL_INTERVAL_S, ARM_BEFORE_MIN, FINALISE_GRACE_H, SCHEDULE_MAX_MEM`.
**CMS**: see cms.md.
